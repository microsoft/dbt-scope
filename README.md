...
| `starting_timestamp_fallback_to_latest` | `false`                     | When `true` and `starting_timestamp` is after every available source file, process only the single most-recent file (minimum lookback) instead of raising — useful for exploring with the least data scanned |
